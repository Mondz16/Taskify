      ...
      <div className="max-w-5xl mx-auto">
        ...
        {loading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="aspect-square bg-slate-800/60 rounded-2xl animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid md:grid-cols-5 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            {boards.map((board, i) => (
              ...
              >
                <div className={`absolute inset-0 bg-gradient-to-br ${BOARD_COLORS[i % BOARD_COLORS.length]}`} />
                ...
                <div className="relative h-full flex flex-col justify-between p-4">
                  ...
                  <p className="text-white font-light text-xs flex-1 mt-2">{format(board.createdAt, "MMMM do, yyyy")}</p>
            ...
            {/* Create new board — inline in grid */}
            {showInput ? (
              <div className="aspect-square bg-slate-900 border border-amber-400/40 rounded-2xl p-4 shadow-xl">
                <form onSubmit={createBoard} className="flex flex-col h-full gap-3">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">New Board</p>
                  <input
                    type="text"
                    autoFocus
